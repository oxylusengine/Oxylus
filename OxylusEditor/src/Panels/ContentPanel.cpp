  ...
  if (ImGui::BeginTable("BodyTable", column_count, flags)) {
    ...
    for (auto& file : self.directory_entries_) {
      ...
      if (grid) {
        ...
        auto thumbnail_image_view = TextureView{};
        if (use_thumbnail_image) {
          if (file.type == FileType::Texture) {
            thumbnail_image_view = editor.thumbnail_manager.get_thumbnail_texture(file_path_str);
          } else if (file.type == FileType::Model) {
            thumbnail_image_view = editor.thumbnail_manager.get_thumbnail_model(file_path_str);
          }
        }
        if (use_thumbnail_image) {
          if (thumbnail_image_view) {
            UI::image(thumbnail_image_view, {thumb_image_size, thumb_image_size});
          } else {
