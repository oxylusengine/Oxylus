...
protected:
  ...
  void TearDown() override { auto _ = network_manager->deinit(); }
...
TEST_F(NetworkingTest, TestServerRecievePacket) {
  ...
  ox::Packet packet(packet_id);
  packet //
    .add_string(packet_string_value)
    .add<ox::u32>(packet_u32_value);
  ...
  auto packet_send_result = client.send_packet(packet);
