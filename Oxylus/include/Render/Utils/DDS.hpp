...
// @ https://github.com/spnda/dds_image/tree/main

#include "DDSFormats.hpp"
...
DDS_NO_DISCARD inline VkFormat getVulkanFormat(DXGI_FORMAT format, const bool alphaFlag) {
  switch (format) {
    ...
    case DXGI_FORMAT_R8G8_UINT     : return VK_FORMAT_R8G8_SINT;
...
DDS_NO_DISCARD inline ReadResult readImage(std::uint8_t* ptr, std::size_t fileSize, dds::Image* image) {
  ...
  auto computeMipmapSize = [bitsPerPixel,
                            blockSizeBytes](DXGI_FORMAT dxgiFormat, uint32_t width, uint32_t height) -> uint32_t {
